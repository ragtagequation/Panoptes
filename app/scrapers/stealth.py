...
def get_proxy() -> Optional[str]:
    proxy = _env('PANOPTES_PROXY')
    if proxy:
        return proxy

    proxy_file = _env('PANOPTES_PROXY_FILE')
    ...
    if _env('PANOPTES_FREE_PROXY', default='').lower() in ('1', 'true', 'yes'):
...
def proxy_status() -> str:
    if _env('PANOPTES_PROXY'):
        return 'custom'
    if _env('PANOPTES_PROXY_FILE'):
        return 'file'
    if _env('PANOPTES_FREE_PROXY', default='').lower() in ('1', 'true', 'yes'):
        return 'free'
    return 'none'
