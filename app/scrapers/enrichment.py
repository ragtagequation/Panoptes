...
class LeadEnricher:
    ...
    def enrich_lead(self, lead_data: Dict) -> Dict:
        ...
        if not email_candidates or not enriched.get('phone'):
            try:
                from app.providers.apollo import apollo_available, find_person
                if apollo_available():
                    domain = work_domain or (self._extract_domain(website) if website else None)
                    hit = find_person(
                    ...
                    if hit:
                        if hit.get('email'):
                            email_candidates.append((hit['email'], 'apollo'))
                        if hit.get('phone') and not enriched.get('phone'):
                            enriched['phone'] = hit['phone']
            except Exception as e:
                logger.debug(f"Apollo enrich skipped: {e}")
    ...
    def _deep_scrape_website(self, website: str) -> Dict:
        ...
        # Prefer Firecrawl when configured
        try:
            from app.providers.firecrawl import firecrawl_available, scrape_url
            if firecrawl_available():
                fc = scrape_url(website)
                if fc:
                    blob = " ".join([fc.get("text") or "", fc.get("html") or ""])
                    emails = [e for e in re.findall(EMAIL_RE, blob) if self._is_valid_email(e)]
                    result['all_emails'] = list(dict.fromkeys(emails))
                    if emails:
                        result['email'] = emails[0]
                    phone = self._extract_phone_from_text(blob)
                    if phone:
                        result['phone'] = phone
                    if result['email'] or result['phone']:
                        return result
        except Exception as e:
            logger.debug(f"Firecrawl deep scrape skipped: {e}")
    ...
    def _verify_email_smtp(self, email: str) -> Dict:
        ...
        try:
            with smtplib.SMTP(timeout=10) as smtp:
                ...
                smtp.helo('panoptes-verify.local')
                smtp.mail('verify@panoptes-verify.local')
